...
class Transformer(nn.Module):
    ...
    def __init__(
        ...
        *,
        ...
        self.mut_embed = nn.Linear(1, d_token)
        ...
        self.last_normalization = nn.LayerNorm(2*d_token) if prenormalization else None
        self.ffn_dropout = ffn_dropout
        self.residual_dropout = residual_dropout
        self.head = nn.Linear(2*d_token, d_out)
        self.gex_in = nn.Identity()
    ...
    def forward(self, x_num: Tensor, x_cat: Tensor) -> Tensor:
        x_gex = self.tokenizer(x_num).float()
        x_mut = x_cat.float()
        x_mut=self.mut_embed(x_mut[:,:,None])
        ...
        """ Cross attention between GEX and mutation"""

        cross_attention_layer = ty.cast(ty.Dict[str, nn.Module], self.cross_attention_layer)
        x_mut = cross_attention_layer['norm0'](x_mut)
        cross_x = cross_attention_layer['attention'](
            x_gex[:, :1],
            x_mut,
            x_mut,
            None,
            None,
            None)
        cross_x = cross_attention_layer['linear0'](cross_x)
        cross_x = self.activation(cross_x)
        if self.ffn_dropout:
            cross_x = F.dropout(cross_x, self.ffn_dropout, self.training)
        cross_x = cross_attention_layer['linear1'](cross_x)

        assert x.shape[1] == 1
        assert cross_x.shape[1] == 1

        x = torch.cat([x[:, 0],cross_x[:, 0]],dim=1)
...
if __name__ == "__main__":
    ...
    # X_num is gex modality and X_cat is mutation modality
    # assert two modalities have the same shape of inputs
    assert X_num['train'].shape == X_cat['train'].shape
    model = Transformer(
