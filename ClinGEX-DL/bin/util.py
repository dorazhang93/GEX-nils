...
def load_config(
    ...
    # parser.add_argument('-c','--config', default='/home/avesta/daqu/Projects/GEX/code/ClinTab-DL/configs/weight_decay_tune/gex_transformer_no_early_stop_1.toml',metavar='FILE')
    parser.add_argument('-d', '--dataset',default='') # gex_midtrain/cancerPathway_sw_norm
    parser.add_argument('-o', '--output', default='',metavar='DIR') # /home/avesta/daqu/Projects/GEX/code/ClinTab-DL/output/test
    parser.add_argument('-f', '--force', default=True,action='store_true')
    parser.add_argument('--continue', action='store_true', dest='continue_')
    parser.add_argument('-e', '--endpoint',default='',type=str)
    parser.add_argument('-r', '--repeat', default=0, type=int)
    parser.add_argument('-kf','--KFold', default=0, type=int)
    parser.add_argument('-pwy','--pathway', default='', type=str)
    if argv is None:
        argv = sys.argv[1:]
    ...
    print(sep, f'Config: {config_path}',f'Dataset: {args.dataset}', f'pathway: {args.pathway}', f'Endpoint: {args.endpoint}', f'Output: {output_dir}',
          sep, sep='\n')

    config = load_toml(config_path)

    assert args.KFold is not None
    assert args.repeat is not None
    assert args.endpoint is not None
    config["kfold"] = args.KFold
    config["repeat"] = args.repeat
    config["endpoint"] = args.endpoint.replace("@"," ")
    config["dataset"] = args.dataset
    config['pathway'] = args.pathway
