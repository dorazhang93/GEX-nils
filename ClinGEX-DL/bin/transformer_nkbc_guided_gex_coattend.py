...
class Transformer(nn.Module):
    ...
    def __init__(
        ...
        *,
        ...
        self.tokenizer_gex = Tokenizer(d_gex_numerical, None, d_token, token_bias)
        self.tokenizer_nkbc = Tokenizer(d_nkbc_numerical, categories, d_token, token_bias)
        n_gex_tokens = self.tokenizer_gex.n_tokens
        n_nkbc_tokens =self.tokenizer_nkbc.n_tokens



        self.shared_kv_compression = None
        ...
        self.cross_attention_layer = nn.ModuleDict(
            {

                'attention': MultiheadAttention(
                    d_token, n_heads, attention_dropout, initialization,
                ),
                'norm0': make_normalization(),
                'linear0': nn.Linear(
    ...
    def forward(self, x_gex: Tensor, x_nkbc_num: Tensor, x_nkbc_cat: ty.Optional[Tensor]) -> Tensor:
        x_gex = self.tokenizer_gex(x_gex,None).float()
        x_nkbc = self.tokenizer_nkbc(x_nkbc_num, x_nkbc_cat).float()
        ...
        """ clinical guided coattend """
        coattend_layer = ty.cast(ty.Dict[str, nn.Module], self.cross_attention_layer)

        x = coattend_layer['attention'](
            x_nkbc,
            x_gex,
            rand_mask,
            *self._get_kv_compressions(coattend_layer),
        )

        x_residual = self._start_residual(x, coattend_layer, 0)
        x_residual = coattend_layer['linear0'](x_residual)
        x_residual = self.activation(x_residual)
        if self.ffn_dropout:
            x_residual = F.dropout(x_residual, self.ffn_dropout, self.training)
        x_residual = coattend_layer['linear1'](x_residual)
        x = self._end_residual(x, x_residual, coattend_layer, 1)

        """ last self attention """
        last_layer = ty.cast(ty.Dict[str, nn.Module], self.last_layer)

        x_residual = self._start_residual(x, last_layer, 0)
        x_residual = last_layer['attention'](
            # for the last attention, it is enough to process only [CLS]
            (x_residual[:, :1]),
            x_residual,
            rand_mask,
            *self._get_kv_compressions(last_layer),
        )
        x = x[:, : x_residual.shape[1]]
        x = self._end_residual(x, x_residual, last_layer, 0)

        x_residual = self._start_residual(x, last_layer, 1)
        x_residual = last_layer['linear0'](x_residual)
        x_residual = self.activation(x_residual)
        if self.ffn_dropout:
            x_residual = F.dropout(x_residual, self.ffn_dropout, self.training)
        x_residual = last_layer['linear1'](x_residual)
        x = self._end_residual(x, x_residual, last_layer, 1)
...
if __name__ == "__main__":
    ...
    print(D.info['C'])
    # construct X=(N,C)
    X_num = {'test': D.test['N']['data'], 'val': D.val['N']['data'], 'train': D.train['N']['data']}
    X_cat = {'test': D.test['C']['data'], 'val': D.val['C']['data'], 'train': D.train['C']['data']}
    ...
    # construct GEX and NKBC data:  N=gex[19657]+nkbc_N[5]; C=nkbc_C[6]
    n_gex=19657
    n_nkbc_num=5 # the last is noise predictor
    n_nkbc_cat=6

    assert X_num['train'].shape[1] == (n_gex + n_nkbc_num)
    assert X_cat['train'].shape[1] == n_nkbc_cat
    X_gex = {k: v[:,:n_gex] for k,v in X_num.items()}
    X_nkbc_num = {k: v[:,n_gex:] for k,v in X_num.items()}
    X_nkbc_cat =X_cat
    del X_num, X_cat
    ...
    model = Transformer(
        d_gex_numerical=X_gex['train'].shape[1],
        d_nkbc_numerical=X_nkbc_num['train'].shape[1],
        categories=get_categories(X_nkbc_cat),
        d_out=n_classes,
        **args['model'],
    ).to(device)
    ...
    def apply_model(part, idx):
        return model(X_gex[part][idx], X_nkbc_num[part][idx],X_nkbc_cat[part][idx],)
