...
class GEXTokenizer(nn.Module):
    ...
    def __init__(
        ...
        self.weight = nn.Parameter(Tensor(n_token, d_token))
        self.bias = nn.Parameter(Tensor(n_token, d_token))
        # The initialization is inspired by nn.Linear
        nn_init.kaiming_uniform_(self.weight, a=math.sqrt(5))
        nn_init.kaiming_uniform_(self.bias, a=math.sqrt(5))
...
class Transformer(nn.Module):
    ...
    def __init__(
        ...
        *,
        # tokenizer
        n_token_list: ty.Optional[ty.List[int]],
        categories: ty.Optional[ty.List[int]],
        token_bias: bool,
        # transformer
        n_layers: int,
        ...
        n_gex, n_nkbc_num = n_token_list
        self.n_gex = n_gex
        self.gex_tokenizer = GEXTokenizer(n_gex, d_token)
        self.nkbc_tokenizer = NKBCTokenizer(n_nkbc_num,categories,d_token)
        self.cls_tokenizer = nn.Parameter(Tensor(1, d_token))
        nn_init.kaiming_uniform_(self.cls_tokenizer, a=math.sqrt(5))

        n_tokens = 1+self.gex_tokenizer.n_tokens+self.nkbc_tokenizer.n_tokens
    ...
    def forward(self, x_num: Tensor, x_cat: ty.Optional[Tensor]) -> Tensor:
        gex = x_num[:,:self.n_gex]
        nkbc_num = x_num[:,self.n_gex:]
        cls = torch.ones(len(gex),1, device=x_num.device)
        cls = self.cls_tokenizer[None] * cls[:,:,None]
        # print('cls weight', self.cls_tokenizer)
        # print('nkbc tokenizer', self.nkbc_tokenizer.weight)
        # print('gex tokenizer', self.gex_tokenizer.weight)

        x = torch.cat(
            [cls,
             self.gex_tokenizer(gex),
             self.nkbc_tokenizer(nkbc_num,x_cat)],
            dim=1,
        ).float() # n,l,d

        x=self.to_embedding(x)# Identity
...
if __name__ == "__main__":
    ...
    pretrained_ckpt_file= args['training']['pretrain_ckpt']
    assert pretrained_ckpt_file is not None and os.path.exists(pretrained_ckpt_file)

    finetune_params={}
    scratch_params= {}
    # load pretrained weights and set gradient stop
    def initialize_and_freeze_model(pretrain_params, pretrain_freeze):
        pretrain_params = pretrain_params.split(',')
        initializations = {}
        for k, v in torch.load(pretrained_ckpt_file)['state_dict'].items():
            print("from pretrained ckpt",k,v.data.shape)
            if any(x in k for x in pretrain_params):
                if 'tokenizer' in k:
                    initializations["gex_"+".".join(k.split(".")[3:])] = v.data #TODO
                else:
                    initializations[".".join(k.split(".")[3:])] = v.data
        print("renamed loaded initializations", initializations.keys())

        gene_ids = np.loadtxt("/home/avesta/daqu/Projects/GEX/GEX_processed/gex/scanb_geo_over_nkbc/gene_ensgs.txt",dtype=str)
        topk_genes = np.loadtxt("/home/avesta/daqu/Projects/GEX/GEX_processed/gex/scanb_geo/cancerpathway_Gradcam_over_noise_repeat10.txt", dtype=str)
        gene_idx = np.isin(gene_ids, topk_genes)

        # Use torch.no_grad() to prevent gradient tracking during initialization
        with torch.no_grad():
            for name, param in model.named_parameters():
                print("from model", name, param.data.shape)
                # Initialize parameter if its name is in the initializations dict
                if name in initializations:
                    values=initializations[name]
                    print(f"✅ Initializing '{name}'...")
                    print(param.data.shape, values.shape)
                    print("before copy, param data", param)
                    if 'tokenizer.weight' in name:
                        # param.data.copy_(values[1:,:]) #TODO
                        assert len(gene_idx)==values.shape[0] and gene_idx.sum()==param.data.shape[0]
                        param.data.copy_(values[gene_idx])
                    elif 'tokenizer.bias' in name:
                        assert len(gene_idx)==values.shape[0] and gene_idx.sum()==param.data.shape[0]
                        param.data.copy_(values[gene_idx])
                    else:
                    ...
                    finetune_params[name]=param
                    print("after copy", param)
                else:
                    scratch_params[name]=param
        print("Finetune parameters", finetune_params.keys())
        print("From scratch parameters", scratch_params.keys())


    initialize_and_freeze_model(args['training']['pretrain_params'], args['training']['pretrain_freeze'])
    ...
    parameters_with_wd = [v for k, v in scratch_params.items() if needs_wd(k)]
    parameters_without_wd = [v for k, v in scratch_params.items() if not needs_wd(k)]

    if args['training']['pretrain_freeze']:
        ...
        finetune_params = [v for k, v in finetune_params.items()]
        optimizer = make_optimizer(
            args['training']['optimizer'],
            (
                [
                    {'params': finetune_params,'lr':1e-5, 'weight_decay': 0.0},
                    {'params': parameters_with_wd},
                    {'params': parameters_without_wd, 'weight_decay': 0.0},
                ]
            ),
