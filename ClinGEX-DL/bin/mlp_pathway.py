...
class MLP(nn.Module):
    def __init__(
        ...
        *,
        ...
        if d_embedding>1:
            pathway_matrix=pathway_matrix.repeat(1,d_embedding)# The number of times to repeat this tensor along each dimension

        # self.sparse_nnlinear = SparseLinear(n_input_feature,n_output_feature,pathway_matrix)
        self.sparse_nnlinear = CustomizedLinear(pathway_matrix)

        d_in = self.d_embedding * self.pathway_num
    ...
    def forward(self, x_num, x_cat):
        ...
        if self.d_embedding>1:
            x = F.dropout(F.relu(self.sparse_nnlinear(x_num)), 0.2,
                      self.training)  # output shape: batch_num * (path_hidden_d*pathway_num)
        else:
...
pathway_matrix = np.load(
    f"/home/avesta/daqu/Projects/GEX/GEX_processed/modeling_data/gex_midtrain/cancerPathway_sw_norm/sparse_NN_mask.npy")
