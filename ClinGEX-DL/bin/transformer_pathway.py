...
class Transformer(nn.Module):
    ...
    def __init__(
        ...
        *,
        ...
        self.pathway_num=pathway_matrix.size(1)
        #
        # pathway_hidden_d=d_token
        # pathway_matrix=pathway_matrix.repeat(1,pathway_hidden_d)# The number of times to repeat this tensor along each dimension
        # self.cls_weight = nn.Parameter(Tensor( 1, d_token))
        # nn_init.kaiming_uniform_(self.cls_weight, a=math.sqrt(5))

        self.sparse_nnlinear = CustomizedLinear(pathway_matrix)
        self.tokenizer = Tokenizer(self.pathway_num,d_token,bias=True)

        n_tokens = self.pathway_num + 1
    ...
    def forward(self, x_num: Tensor, x_cat) -> Tensor:
        # # spare linear connection
        # x = F.dropout(F.relu(self.sparse_nnlinear(x_num)), 0.2, self.training) # output shape: batch_num * (path_hidden_d*pathway_num)
        #
        # n_batch = x.shape[0]
        # # reshape to n, num_pathway, d_token
        # x = x.reshape(n_batch, self.d_token,self.pathway_num).transpose(1,2)
        # x_cls = self.cls_weight[None] * torch.ones(n_batch,1,1, device=x_num.device)
        # # concat x_cls ans pathway embedding
        # x = torch.cat([x_cls,x], dim=1)

        x= F.relu(self.sparse_nnlinear(x_num))
        x= self.tokenizer(x)
...
if __name__ == "__main__":
    ...
    pathway_matrix=np.load(f"/home/avesta/daqu/Projects/GEX/GEX_processed/modeling_data/gex_midtrain/cancerPathway_sw_norm/sparse_NN_mask.npy")
    pathway_matrix=torch.as_tensor(pathway_matrix)
    print("Pathway matrix", pathway_matrix.shape)
    print("number of connections", pathway_matrix.sum())
    model = Transformer(
