...
class Tokenizer(nn.Module):
    ...
    def __init__(
        ...
        super().__init__()
        num_token=d_numerical*d_channel
        self.n_tokens = d_numerical +1
        self.d_token = d_token*d_channel

        # take [CLS] token into account
        self.weight_cls = nn.Parameter(Tensor(1, d_token*d_channel))
        self.weight = nn.Parameter(Tensor(num_token, d_token))
        self.bias = nn.Parameter(Tensor(num_token, d_token)) if bias else None
    ...
    def forward(self, x_num: Tensor) -> Tensor:
        assert len(x_num.shape)==3 # n,l,d
        n, l, _ = x_num.shape
        x_num = x_num.reshape(len(x_num), -1) # n, ld
        x_num = self.weight[None] * x_num[:, :, None] # n, ld, d_token
        x_cls = self.weight_cls[None] * torch.ones(len(x_num), 1, 1, device=x_num.device)  # [CLS] n, 1, d_token*d

        if self.bias is not None:
            x_num = x_num + self.bias[None]

        x_num = x_num.reshape(n, l, -1) # n, l, d_token*d

        x = torch.cat([x_cls,x_num],dim=1) # n, 1+l, d_token*d
...
class Transformer(nn.Module):
    ...
    def __init__(
        ...
        *,
        ...
        super().__init__()
        self.tokenizer = Tokenizer(d_numerical, feature_channel, d_token, token_bias)
        n_tokens = self.tokenizer.n_tokens
        d_token = self.tokenizer.d_token
        print(f"Tokenizer d_token={d_token}, n_token={n_tokens}")
    ...
    def forward(self, x_num: Tensor, x_cat: ty.Optional[Tensor]) -> Tensor:
        ...
        x = self.tokenizer(x_num).float() # n, n_token, d_token
...
if __name__ == "__main__":
    ...
    args['model'].setdefault('token_bias', False)
    ...
    model = Transformer(
        ...
        feature_channel=0 if X_num is None else X_num['train'].shape[2],
        d_out=n_classes,
        **args['model'],
    ).to(device)
    ...
    if args['training']['pretrain_ckpt']:
        ...
        pretrained_weights={}
        for k, v in torch.load(args['training']['pretrain_ckpt'])['state_dict'].items():
            if 'encoder.tokenizer' in k:
                pretrained_weights[".".join(k.split(".")[2:])]=v
