...
class SparseLinear(nn.Module):
    def __init__(self, in_features, out_features, mask):
        ...
        mask = mask.t()
        self.weight = nn.Parameter(data=torch.sparse.FloatTensor(indices=torch.nonzero(mask).T,
                                                                 values=torch.randn(mask.sum()),
    ...
    def forward(self, x):
        print("SparseLinear bias",self.bias[:10])
        print("SparseLinear weight",self.weight)
...
def make_lr_schedule(
    ...
    if lr_schedule['type'] in ['transformer', 'linear_warmup']:
        ...
        lr_scheduler = optim.lr_scheduler.ExponentialLR(optimizer,gamma=lr_schedule['scheduler_gamma'])
...
def train_with_auto_virtual_batch(
    ...
    batch_size = len(batch)
    random_state = zero.get_random_state()
    # posw = torch.FloatTensor([5.0]).to(get_device())# TODO
    ...
    while chunk_size != 0:
        try:
            ...
            if batch_size <= chunk_size:
                if alpha is None:
                    # loss = loss_fn(*step(batch),pos_weight=posw)#TODO
                    loss = loss_fn(*step(batch))
                else:
                    loss = loss_fn(*step(batch), alpha= alpha, gamma=gamma)
                loss.backward()
            else:
                loss = None
                for chunk in zero.iter_batches(batch, chunk_size):
                    if alpha is None:
                        # chunk_loss = loss_fn(*step(chunk),pos_weight=posw)#TODO
                        chunk_loss = loss_fn(*step(chunk))
                    else:
                        chunk_loss = loss_fn(*step(chunk), alpha= alpha, gamma=gamma)
...
def triplet_ce_loss(
    ...
    idx_pos = (targets==1.0).nonzero(as_tuple=True)[0]
    idx_neg = (targets==0.0).nonzero(as_tuple=True)[0]
    pos = features[idx_pos].contiguous()
    neg = features[idx_neg].contiguous()
    len_pos = len(pos)
    len_neg = len(neg)
    if len_pos < 8:
        print(f"{len_pos} positive samples, less than 8!!!")
        return ce_loss
    pos_samples_idxs= torch.cat([Tensor([random.sample(range(len_pos),2)]) for i in range(logits.shape[0])], axis=0)
    anchor_indexs, positive_indexs = pos_samples_idxs.unbind(-1)
    neg_samples_idxs= torch.cat([Tensor(random.sample(range(len_neg),1)) for i in range(logits.shape[0])], axis=0)
    anchors= pos[anchor_indexs.long()]
    positives = pos[positive_indexs.long()]
    negtives = neg[neg_samples_idxs.long()]
    triplet_loss= nn.TripletMarginLoss(margin=margin,p=p)
    tri_loss = triplet_loss(anchors,positives,negtives)
    return (ce_loss+tri_loss)/2
