...
def objective(trial: optuna.trial.Trial) -> float:
    ...
    with tempfile.TemporaryDirectory() as dir_:
        ...
        val_score=[]
        for k in range(5):
            out = dir_ / f'trial_{trial.number}_K{k}'
            ...
            try:
                ...
                stats['algorithm'] = program.stem.split("/")[-1]
                metrics={}
                metrics["train"] = {'score': -1.0}
                metrics["val"] = {'score': -1.0}
            ...
            lib.backup_output(output)
            print(f'Time: {lib.format_seconds(timer())}')
            val_score.append(stats['metrics'][lib.VAL]['score'])
...
# multiplyer 5 means 5 fold cross validation
stats['best_stats'] = trial_stats[best_trial_id*5]
